# (1) Raw 이미지 파일을 선택해서 DB에 업로드하는 python 코딩
# 서버 - Fedora22
# DB - photoDB-photoTBL(id(자동), fName, fType(jpg, raw,png ...), image)
# (2) Raw 이미지의 리스트를 화면에서 조회하고, 더블클릭하면 서브 윈도에 출력하기
# (3) 심화
# RAW 이미지 처리 프로그램을 [파일]-[열기] 아래 [파일]-[DB에서 불러오기]를 추가해라 => 입력값은 파일이름(fName)
# (4) 심화 [파일]-[DB에 업로드하기]를 추가
...
fileMenu.add_cascade(label='DB에서 열기', command=None) # OnDB
