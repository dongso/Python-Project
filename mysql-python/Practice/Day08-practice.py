...
def csv2raw():
    ...
    with open(fname, 'r', newline='') as fileReader:
        ...
        row_count = sum(1 for row in csvReader)-1  # fileObject is your csv.reader
        print(row_count)

        hei=int(math.sqrt(row_count)-1)
        ...
        for rowList in csvReader:  # csv파일의 내용을 추가한다.
            tempList[rowList[0]][rowList[1]]=rowList[2]
...
rawfileMenu.add_command(label="테이블에 저장하기", command=rawToDB)
rawfileMenu.add_command(label="CSV에서 읽어오기", command=rawInDB)
