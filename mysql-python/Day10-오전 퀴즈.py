...
def toColorImageCV(photo2, scale=1):
...
def grayCV2():
    ...
    cvPhoto2=cv2.cvtColor(cvPhoto, cv2.COLOR_BGR2GRAY)
    photo2 = Image.fromarray(cvPhoto2)
    ###########################################################
...
def blurringCV2():
    ...
    bValue=askinteger("블러링 크기", "입력해주세요")

    ####### 블러링을 CV2 메소드로 구현하기 --> photo2로 넘기기 ####
    cvPhoto2 = cvPhoto[:]  # 복사
    mask = np.zeros((bValue, bValue), np.float32);
    for i in range(bValue):
        for j in range(bValue):
            mask[i][j]=1/pow(bValue,2)
...
def rotateCV2():
    ...
    ratate_matrix = cv2.getRotationMatrix2D((inW/2, inH/2), angle,1) #(기준점), 각도, 스케일
    cvPhoto2 = cv2.warpAffine(cvPhoto2, ratate_matrix, (inW,inH))
    photo2=Image.fromarray(cvPhoto2)
...
def reSizeCV2():
    ...
    scale=askfloat("스케일", "입력하세용", minvalue=0.1, maxvalue=10.0)
...
def waveCV2():
    ...
    for i in range(inH):
        for j in range(inW):
            ...
            if i+offset_y < inH and j+offset_x < inW:
                cvPhoto2[i,j] = cvPhoto[(i+offset_y)%inH,(j+offset_x)%inW]
            else:
                cvPhoto2[i,j] = 0
...
def mirrorCV2():
    ...
    for i in range(inH):
        for k in range(inW):
            cvPhoto2[i][k]=cvPhoto[i][inW-k-1]
...
def mirror2CV2():
    ...
    for i in range(inH):
        for k in range(inW):
            cvPhoto2[i][k]=cvPhoto[inH-i-1][k]
    ...
    return
## 메인 코드부
window = Tk(); window.title('빅데이터 분석/처리 통합 툴 (Ver 0.07)')
...
openCVMenu.add_command(label="그레이스케일(OpenCV)", command=grayCV2)
openCVMenu.add_command(label="블러링(OpenCV)", command=blurringCV2)
openCVMenu.add_command(label="이동(OpenCV)", command=rotateCV2)
openCVMenu.add_command(label="크기변환(OpenCV)", command=reSizeCV2)
openCVMenu.add_separator()
openCVMenu.add_command(label="수평Wave", command=waveVirtCV2)
openCVMenu.add_command(label="수직Wave", command=waveHoriCV2)
openCVMenu.add_command(label="Wave", command=waveCV2)

openCVMenu.add_separator()
openCVMenu.add_command(label="카툰화", command=cartoonCV2)
openCVMenu.add_command(label="미러링(좌우)", command=mirrorCV2)
openCVMenu.add_command(label="미러링(상하)", command=mirror2CV2)
