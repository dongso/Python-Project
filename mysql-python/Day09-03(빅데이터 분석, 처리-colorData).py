    ...
    for i in range(inH) :
        for k in range(inW) :
            if inImageR[i][k] + value > 255 :
                outImageR[i][k] = 255
            else : outImageR[i][k]=inImageR[i][k]+value

            if inImageG[i][k] + value> 255 :
                outImageG[i][k]=255
            else : outImageG[i][k]=inImageG[i][k]+value

            if inImageB[i][k]+value >255:
                outImageB[i][k]=255
            else : outImageB[i][k]=inImageB[i][k]+value
...
def mirror1ImageColor() :
    ...
    for i in range(inH) :
        for k in range(inW) :
            outImageR[inW-1-i][k] = inImageR[i][k]
            outImageG[inW-1-i][k] = inImageG[i][k]
            outImageB[inW-1-i][k] = inImageB[i][k]
    ################################
    displayImageColor()

def zoomIn1ImageColor() :
    global window, canvas, paper, inW, inH, outW, outH, inImageR, inImageG, inImageB, outImageR, outImageG, outImageB, filename
    outImageR, outImageG, outImageB = [], [], []  # 초기화
...
colorDataMenu.add_command(label="어둡게하기", command=None)
colorDataMenu.add_command(label='이진화', command=bwImageColor)
colorDataMenu.add_command(label='반전시키기', command=reverseImageColor)
colorDataMenu.add_command(label='상하미러링', command=mirror1ImageColor)
colorDataMenu.add_command(label="좌우미러링", command=mirror1ImageColor)
colorDataMenu.add_command(label="영상확대", command=zoomIn1ImageColor)
colorDataMenu.add_command(label="영상축소", command=zoomOut1ImageColor)
colorDataMenu.add_command(label ="영상 분포도 확인", command=matHistoRAWColor)
