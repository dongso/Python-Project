...
def loadDB() :
    ...
    sql = "SELECT distinct (fname), fType FROM colorTBL"  # ID로 추출하기
    cur.execute(sql)
...
def sheetDblClick(event) :
    ...
    entNum = int(item[1:],16) - 1  ## 쿼리한 결과 리스트의 순번( hex to dec)
    fname = rows[entNum][0] ## 선택한 id
    window2.destroy()
    # DB에서 이미지를 다운로드
    con,cur= dbCONN()

    sql = "SELECT fname, x,y,r FROM colorTBL WHERE fname='" + str(fname)+"'" # ID로 이미지 추출하기
    cur.execute(sql)
    rows=cur.fetchall()
    ...
    for row in rows:
        cList[int(row[1])][int(row[2])] = int(row[3])
...
def makeEmptyRGBList():
    ...
    for _ in range(inH):
        ...
        for _ in range(inW):
            tmp.append(0)
        inImageB.append(tmp)
    return inImageR,inImageG, inImageB
...
def color2DB():
    ...
    for i in range(outH):
        for k in range(outW):
            sql="insert into colortbl(fname, ftype, xSize, ySize, x, y, r, g, b) values('"+fname+"','"+fname.split('.')[1]+"',"
            sql+=str(outW)+","+str(outH)+","+str(i)+", "+str(k)+","+str(outImageR[i][k])+","+str(outImageG[i][k])+","+str(outImageB[i][k])+")"
            cur.execute(sql)
            #print(sql)
            #break
        #break
...
def sheetDblClick3(event) :
    ...
    con = pymysql.connect(host=IP_ADDR, user=USER_NAME, password=USER_PASS, database=DB_NAME, charset='utf8')
    cur = con.cursor()
    sql = "SELECT x, y, xSize, ySize, r, g, b FROM colorTBL WHERE fName='" + fileID + "'" # ID로 이미지 추출하기
    cur.execute(sql)
    ...
    ### 입력 영상을 완성 ### (inImage, inW, inH)
    #print(colorRows)
    inW = colorRows[entNum][2]; inH=colorRows[entNum][3]
    inImageR,inImageG, inImageB = [],[],[]

    inImageR,inImageG, inImageB=makeEmptyRGBList()

    for row in colorRows :
        x, y, xSize, ySize, r, g, b = row
        inImageR[x][y] = r
        inImageG[x][y] = g
        inImageB[x][y] = b
...
def setMask(inMask,inSize,plus=FALSE):
    global window, canvas, paper, inW, inH, outW, outH, inImage, outImage
    outImage = []  # 초기화
    # outImage의 크기를 결정
    outH = inH;
    ...
    # 임시 입력 영상 -> boundary 문제 해결 (기존 이미지보다 2씩 더 큼)
    tmpInImage = []
    for _ in range(inH + 2):
        tmp = []
        for _ in range(inW + 2):
            tmp.append(127)  # 중간값으로 초기화
    ...
    for i in range(1, inH):
        for k in range(1, inW):
            # 한 점을 처리하기 (3x3 반복 수행). 같은 위치끼리 곱한 뒤 합을 구함.
            S = 0.0
    ...
    for i in range(outH):
        for k in range(outW):
            value = inImage[i][k] - int(tmpOutImage[i][k])
            if value > 255:
                value = 255
            elif value < 0:
                value = 0
...
def loadImageColorCV2(fname) :
    global window, canvas, paper, inW, inH, outW, outH, inImageR, inImageG, inImageB, outImageR, outImageG, outImageB, filename, photo, cvPhoto
    inImageR, inImageG, inImageB = [], [], [] # 초기화
    outImageR, outImageG, outImageB = [], [], [] # 초기화
...
def embossingCV() :
    ...
    ###엠보싱을 cv2메소드로 구현하기 -> photo2로 넘기기 ###
    cvPhoto2 = cvPhoto[:]# copy
...
def setMask2CV(inMask,inSize,plus=FALSE):
    global window, canvas, paper, inW, inH, outW, outH, inImage, outImage
    outImage = []  # 초기화
    # outImage의 크기를 결정
    outH = inH;
...
rawImage2Menu.add_command(label="샤프닝", command=sharpening)
rawImage2Menu.add_command(label="블러링", command=blurring)
rawImage2Menu.add_command(label="에지검출1(가로)", command=edge1)
rawImage2Menu.add_command(label="에지검출1(세로)", command=edge2)
...
colorDataMenu.add_command(label="샤프닝", command=sharpeningColor)
colorDataMenu.add_command(label="블러링", command=blurringColor)
colorDataMenu.add_command(label="edge 검출", command=edgeColor)

colorDataMenu.add_separator()
colorDataMenu.add_command(label="DB에 저장하기", command=color2DB)
colorDataMenu.add_command(label="DB에서 불러오기", command=colorInDB)
...
openCVMenu.add_command(label="엠보싱(openCV", command=embossingCV)
openCVMenu.add_command(label="엠보싱(openCV", command=embossingCV)
openCVMenu.add_command(label="엠보싱(openCV", command=embossingCV)
