...
def maskCV2() :
    ...
    ####### CV2 메소드로 구현하기 --> photo2로 넘기기 ####
    cascade = cv2.CascadeClassifier('haarcascade_frontalface_alt.xml')
    faceMask = cv2.imread('D:\ds\Python-Project\mysql-python\images(ML)/mask_hannibal.png')
...
def deepCV2():
    ...
    ####### 딥러닝 기반 적용 ####
    cvPhoto2 = cvPhoto[:]  # 복사
    #args={'image': 'C:/images/deep_images/IMG_8840.jpeg', 'prototxt': 'MobileNetSSD_deploy.prototxt.txt', 'model': 'MobileNetSSD_deploy.caffemodel', 'confidence': 0.2}

    args={'image': filename, 'prototxt': 'MobileNetSSD_deploy.prototxt.txt', 'model': 'MobileNetSSD_deploy.caffemodel', 'confidence': 0.2}
...
## 메인 코드부
window = Tk(); window.title('빅데이터 분석/처리 통합 툴 (Ver 0.14)')
...
openCVMenu.add_command(label="얼굴인식(머신러닝)", command=faceCV2)
openCVMenu.add_command(label="얼굴가면 씌우기(머신러닝)", command=maskCV2)
openCVMenu.add_separator()
openCVMenu.add_command(label="사물인식(딥러닝)", command=deepCV2)
openCVMenu.add_command(label="동영상 인식(딥러닝)", command=videoDeepCV2)
